...
const ResetEmailVerify: React.FC = () => {
  const [code, setCode] = useState<string>("");
  const [error, setError] = useState<string>("");
  const [sending, setSending] = useState<boolean>(false);
  const { currentModal, openModal, closeModal, email, reason } =
    useModals();

  if (currentModal !== "ForgotPwdVerifyCode") return null;
  ...
  const navigateBack = () => {
    openModal("signIn"); // Navigate back to the SignUp modal
  };
...
export default ResetEmailVerify;
